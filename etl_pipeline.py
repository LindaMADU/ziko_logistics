...
product = ziko_df [['Product_ID', 'Quantity','Product_List_Title','Unit_Price', 'Total_Cost', 'Discount_Rate']].copy().drop_duplicates().reset_index(drop=True)


# Create Transaction_Fact_Table
transaction_fact = ziko_df.merge(customer, on=['Customer_ID','Customer_Name','Customer_Phone','Customer_Email', 'Customer_Address'], how='left') \
                     .merge(product, on=['Product_ID', 'Quantity','Product_List_Title','Unit_Price','Total_Cost', 'Discount_Rate'], how='left') \
...
print('files have been loaded temporarily into local machine successfully')
